                                        ...
                                        if(err){console.log(err)}
                                            ...
                                            if(rows.length>0){
                                                ...
                                                res.render('reviewstats',
                                                {
                                                    ...
                                                    myReservations: "/profile/"+req.params.username+"/myreservations",
                                                    hotelTitle: "Reservation Statistics",
                                                    user: req.params.username,
                                                    searchRoute: "/profile/"+req.params.username+"/search",
                                                    hotels: hotels,
                                                    myRoomReviews: "/profile/"+req.params.username+"/myroomreviews/reviews",
        ...
        app.get("/profile/:username/myroomreviews/reviews",function(req,res){
            ...
            connection.query(getCID,[req.params.username],function(err,rows){
                if(err){console.log(err)}
                    ...
                    var query = "select r.rate, r.comment, r.Room_no, h.imagePath, h.Hotel_ID, l.Street, l.City, l.State, l.Country, p.Phone from RoomReview r left join Customer c on c.cid=r.cid left join Hotel h on r.Hotel_ID=h.Hotel_ID left join Location l on h.Hotel_ID=l.Hotel_ID  left join Phones p on p.Hotel_ID = l.Hotel_ID where r.cid=?;"
        ...
        app.get("/profile/:username/mybreakfastreviews/reviews",function(req,res){
            ...
            connection.query(getCID,[req.params.username],function(err,rows){
                if(err){console.log(err)}
                    ...
                    var cid = rows[0].cid;
                    var query = "select r.rate, r.comment, r.bType, h.Hotel_ID, h.imagePath, l.Street, l.City, l.State, l.Country, p.Phone from BreakfastReview r join Customer c on c.cid=r.cid left join Hotel h on r.Hotel_ID=h.Hotel_ID left join Location l on h.Hotel_ID=l.Hotel_ID  left join Phones p on p.Hotel_ID = l.Hotel_ID where r.cid=?";
                    connection.query(query,[cid],function(err,rows){
                        if(err){console.log(err)}
                        else{
                            var reviews = rows;
        ...
        app.get("/profile/:username/myservicereviews/reviews",function(req,res){
            ...
            connection.query(getCID,[req.params.username],function(err,rows){
                if(err){console.log(err)}
                    ...
                    var cid = rows[0].cid;
                    var query = "select r.rate, r.comment, r.sType, h.imagePath, h.Hotel_ID, l.Street, l.City, l.State, l.Country, p.Phone from ServiceReview r join Customer c on c.cid=r.cid left join Hotel h on r.Hotel_ID=h.Hotel_ID left join Location l on h.Hotel_ID=l.Hotel_ID  left join Phones p on p.Hotel_ID = l.Hotel_ID where r.cid=?;"
                    connection.query(query,[cid],function(err,rows){
                        if(err){console.log(err)}
                        else{
                            var reviews = rows;
